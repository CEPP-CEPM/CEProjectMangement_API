...
export class AssignmentSubmitService {
  ...
  async findAllApproveByProctor(assignId: string) {
    return await this.prismaService.assignmentSubmit.findMany({
      where: { assignmentId: assignId, status: 'APPROVE' },
      include: { Groups: true },
    });
  }

  async findAssignSubmitByAvisorId(assignId: string, user: Users) {
    const myGroup = await this.prismaService.groups.findMany({
      where: { createBy: user.id },
    });
    const allAssignSubmit = [];
    await Promise.all(
      myGroup.map(async (group) => {
    ...
    return await Promise.all(
      allAssignSubmit.map(async (assignSubmit) => {
  ...
  async createAssignmentSubmit(
    files: BufferedFile[],
    createAssignmentSubmitDto: CreateAssignmentSubmitDto,
    user: Users,
  ) {
    try {
      // console.log(files)
      ...
      await Promise.all(
        uploadFiles.map(async (file) => {
          const assignmentFile =
            await this.prismaService.assignmentSubmitFiles.create({
              data: {
  ...
  async approveByAdvisor(assignSubmitid: string, user: Users) {
    const assignSubmit = await this.prismaService.assignmentSubmit.findUnique({
      where: { id: assignSubmitid },
    });
    const group = await this.prismaService.groups.findUnique({
      where: { id: assignSubmit.groupId },
    });
    if (group.createBy != user.id) throw new ForbiddenException();
    const usergroup = await this.prismaService.userGroups.findMany({
  ...
  async cancelSubmit(assignmentSubmitid: string, user: Users) {
    ...
    const assignSubmitFile =
      await this.prismaService.assignmentSubmitFiles.findMany({
        where: { assignmentSubmitId: assignmentSubmitid },
      });
    if (assignSubmitFile) {
      await this.deleteFiles(assignSubmitFile);
    }
    return await this.prismaService.assignmentSubmit.delete({
      where: { id: assignmentSubmitid },
    });
  }

  async rejectSubmit(assignmentSubmitid: string) {
    const assignSubmitFile =
      await this.prismaService.assignmentSubmitFiles.findMany({
        where: { assignmentSubmitId: assignmentSubmitid },
      });
  ...
  private async deleteFiles(files: deleteAssignmentsSubmitFiles[]) {
    await Promise.all(
      files.map(async (file) => {
        await this.minioClientService.delete(file.name, file.bucket);
      }),
    );
  }
}
