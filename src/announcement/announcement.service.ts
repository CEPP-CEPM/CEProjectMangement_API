...
export class AnnouncementService {
  ...
  async updateAnnouncement(
    id: string,
    updateAnnouncementDto: Prisma.AnnouncementsUpdateInput,
    files: BufferedFile[],
  ) {
    ...
    const filesInAnnounce = await this.prismaService.announcementFiles.findMany(
      { where: { announcementId: id } },
    );
    await Promise.all(
      filesInAnnounce.map(async (file) => {
        await this.prismaService.announcementFiles.delete({
          where: { id: file.id },
        });
      }),
    );
  ...
  async deleteById(id: string) {
    const findAllAnnouncementFiles =
