...
export class AssignmentService {
      ...
      await Promise.all(
        uploadFiles.map(async (file) => {
          const assignmentFile =
            await this.prismaService.assignmentFiles.create({
              data: {
  ...
  async update(
    id: string,
    updateAssignmentDto: Prisma.AssignmentsUpdateInput,
    files: BufferedFile[],
  ) {
    ...
    const filesInAssign = await this.prismaService.assignmentFiles.findMany({
      where: { assignmentId: id },
    });
    await Promise.all(
      filesInAssign.map(async (file) => {
        await this.prismaService.assignmentFiles.delete({
          where: { id: file.id },
        });
      }),
    );
  ...
  async deleteById(id: string) {
    const findAllAssignmentFiles =
      await this.prismaService.assignmentFiles.findMany({
        where: { assignmentId: id },
      });
